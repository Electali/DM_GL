...
/**
 * REST controller for managing {@link info4.gl.coopcycle.domain.Paiement}.
 */
@RestController
@RequestMapping("/api")
public class PaiementResource {
    ...
    public List<PaiementDTO> getAllPaiements(@RequestParam(required = false) String filter) {
        if ("course-is-null".equals(filter)) {
            log.debug("REST request to get all Paiements where course is null");
            return paiementService.findAllWhereCourseIsNull();
        }
